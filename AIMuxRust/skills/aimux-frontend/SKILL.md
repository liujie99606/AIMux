...
- 每次改动完成并测试通过后，默认保留工作区改动，**不要自动执行 `git commit` 或 `git push`**。
- 只有用户明确要求提交时，才创建中文说明的本地 commit；只有用户明确要求推送时，才执行 `git push`。
- 未经用户明确要求，不要使用 `/commit` 或其他自动提交流程，也不要擅自整理、覆盖或回退用户已有改动。
...
11. 是否按用户要求决定了是否需要 `git commit`；未明确要求时保持改动未提交？
