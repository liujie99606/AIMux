...
class SettingsView(QWidget):
    def __init__(self, client: ApiClient, parent=None) -> None:
        super().__init__(parent); self.client = client; root = QVBoxLayout(self); form = QFormLayout(); root.addLayout(form)
        self.host = QLineEdit(); self.port = QSpinBox(); self.port.setRange(1, 65535); self.db_path = QLineEdit(); self.timeout = QSpinBox(); self.timeout.setRange(1, 3600); self.first_timeout = QSpinBox(); self.first_timeout.setRange(1, 3600); self.retries = QSpinBox(); self.retries.setRange(0, 20); self.token = QLineEdit(); self.token.setEchoMode(QLineEdit.EchoMode.Password); self.launch_at_login = QCheckBox("开机后自动启动 AIMux")
        for label, field in [("监听地址", self.host), ("端口", self.port), ("数据库路径", self.db_path), ("上游超时（秒）", self.timeout), ("首字超时（秒）", self.first_timeout), ("重试次数", self.retries), ("本地令牌", self.token), ("开机自启", self.launch_at_login)]: form.addRow(label, field)
