...
class SettingsView(QWidget):
    def __init__(self, client: ApiClient, parent=None) -> None:
        ...
        self.first_timeout = QSpinBox()
        self.first_timeout.setRange(1, 3600)
        self.retries = QSpinBox()
        self.retries.setRange(1, 20)
        self.proxy_enabled = QCheckBox("启用上游代理")
    ...
    def save(self) -> None:
        payload = {
            ...
            "upstream_timeout_seconds": self.timeout.value(),
            "first_token_timeout_seconds": self.first_timeout.value(),
        ...
        try:
            ...
            self.client.token = payload["local_token"]
            QMessageBox.information(
                self,
                "设置",
                "设置已保存。监听地址、端口和数据库路径将在下次启动时生效；代理和超时设置对后续请求立即生效。",
            )
        except Exception as exc:
