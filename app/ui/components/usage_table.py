...
class UsageTable(QTableWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.setHorizontalHeaderLabels(["时间", "账号", "类型", "模型", "接口", "结果", "耗时", "Token"])
    ...
    def set_records(self, records: list[dict]) -> None:
        ...
        for row, item in enumerate(records):
            values = [item["started_at"], item.get("account_name") or "-", item.get("account_type") or "-", item.get("model") or "-", item.get("endpoint") or "-", "成功" if item["success"] else "失败", f"{item.get('duration_ms') or 0} ms", str(item.get("total_tokens") or 0)]
