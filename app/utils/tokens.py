...
TokenUsage = tuple[int | None, int | None, int | None, int | None]
...
def extract_usage_fields(payload: dict[str, Any] | None) -> TokenUsage:
    ...
    cached_tokens = _as_token_count(usage.get("cached_tokens"))
    for details_key in ("prompt_tokens_details", "input_tokens_details"):
        details = usage.get(details_key)
        if isinstance(details, dict):
            details_cached_tokens = _as_token_count(details.get("cached_tokens"))
            if details_cached_tokens is not None:
                cached_tokens = details_cached_tokens
                break
...
def extract_usage(payload: dict[str, Any] | None) -> TokenUsage:
    ...
    input_tokens, output_tokens, total_tokens, cached_tokens = extract_usage_fields(payload)
